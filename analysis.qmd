...
```{r}
fit_pred <- linear_reg() |> fit(lived_after ~ election_result + win_margin + election_age + party, data = x_pred) |> tidy(conf.int = TRUE)
```

Winning is associated with a strong and significant boost in post-election life expectancy (~9.5 years).
Older age at election consistently predicts shorter post-election lifespan.
Narrow election wins predict longer life after election than large-margin wins.
Republicans tend to live longer than Democrats, even after controlling for other variables.
Third-party candidates show worse outcomes, but uncertainty is high (wide confidence interval, p ≈ 0.08).
