...
```{r}
linear_reg() |> fit(lived_after ~ sex, data = x) |> 
  tidy(conf.int = TRUE)
```

There’s some evidence that men may live longer after an election than women, but the result is not statistically significant. The uncertainty is too large to confidently claim a real difference based on this model.
...
Note that we can't include election_result for predictive model because it is always "Won"!

```{r}
fit_pred <- linear_reg() |> fit(lived_after ~ win_margin + election_age + party, data = x_pred) |> tidy(conf.int = TRUE)
```

Age at election is strongly predictive: Older governors naturally lived fewer years after election.
Party affiliation (Republican vs Democrat) has a notable effect on post-election longevity.
Win margin has no meaningful predictive power here — it doesn’t significantly affect how long they lived after winning.
The effect for third-party candidates is large and negative but with wide confidence intervals — due to likely small sample size.

```{r}
fit_causal <- linear_reg() |> 
  fit(lived_after ~ election_result + win_margin + election_age + party, data = x_causal) |> tidy(conf.int = TRUE)
```

Winning a close election has a positive and significant effect: ~8 more years lived after election.
Older candidates live significantly fewer years after the election.
Republican challengers may live longer than Democrats, even after accounting for win status and age.
Third-party candidates seem to live less, but the evidence is not strong.
